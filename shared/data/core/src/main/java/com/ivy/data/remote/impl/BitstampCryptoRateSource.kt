...
class BitstampCryptoRateSource @Inject constructor(
    ...
    override suspend fun fetchRate(crypto: String, fiat: String): Double? {
        ...
        try {
            val url = "https://www.bitstamp.net/api/v2/ticker/${crypto.lowercase()}${fiat.lowercase()}"
            val response: BitstampResponse = httpClient.get(url).body()
            val rate = response.last.toDoubleOrNull()
            if (rate != null && rate >= 0) {
    ...
    private data class BitstampResponse(val last: String)
} 